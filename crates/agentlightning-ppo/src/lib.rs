...
impl PpoAlgorithm {
    ...
    fn process_batch(&self, spans: &[Span]) -> std::result::Result<Batch, String> {
        ...
        let obs_tensor = Tensor::from_vec(obs_vec, (count, self.input_dim), &self.device)
            .map_err(|e| e.to_string())?;
        let next_obs_tensor = Tensor::from_vec(next_obs_vec, (count, self.input_dim), &self.device)
            .map_err(|e| e.to_string())?;
        let act_tensor =
            Tensor::from_vec(act_vec, (count,), &self.device).map_err(|e| e.to_string())?;
        let rew_tensor =
            Tensor::from_vec(rew_vec, (count,), &self.device).map_err(|e| e.to_string())?;
        let done_tensor =
            Tensor::from_vec(done_vec, (count,), &self.device).map_err(|e| e.to_string())?;
    ...
    fn extract_features(&self, value: &serde_json::Value) -> std::result::Result<Vec<f32>, String> {
        if let Some(arr) = value.get("features").and_then(|v| v.as_array()) {
            ...
            let vec: std::result::Result<Vec<f32>, _> = arr
                .iter()
                .map(|v| v.as_f64().ok_or(()).map(|f| f as f32))
                .collect();
...
impl LightningAlgorithm for PpoAlgorithm {
    async fn train(&mut self, spans: &[Span]) -> Result<Option<TrainingResult>> {
        ...
        let old_log_probs = {
            let (logits, _values) = self
                .model
                .forward(&batch.obs)
                .map_err(|e| Error::Training(format!("Forward: {}", e)))?;
            let probs = self
                .get_log_probs(&logits, &batch.act)
                .map_err(|e| Error::Training(format!("LogProbs: {}", e)))?;
            probs.detach() // Detach from graph explicitly
        };

        // 2. GAE Calculation
        let (_logits, values) = self
            .model
            .forward(&batch.obs)
            .map_err(|e| Error::Training(format!("Forward: {}", e)))?;
        let (_next_logits, next_values) = self
            .model
            .forward(&batch.next_obs)
            .map_err(|e| Error::Training(format!("Next Forward: {}", e)))?;
        ...
        let values_vec = values
            ...
            .to_vec1::<f32>()
            .map_err(|e| Error::Training(e.to_string()))?;
        let next_values_vec = next_values
            .squeeze(1)
            .map_err(|e| Error::Training(e.to_string()))?
            .to_vec1::<f32>()
            .map_err(|e| Error::Training(e.to_string()))?;
        let rewards_vec = batch
            .rew
            .to_vec1::<f32>()
            .map_err(|e| Error::Training(e.to_string()))?;
        let dones_vec = batch
        ...
        for t in (0..batch.size).rev() {
            let delta =
                rewards_vec[t] + GAMMA * next_values_vec[t] * (1.0 - dones_vec[t]) - values_vec[t];
        ...
        // Tensorize
        let adv_tensor = Tensor::from_vec(advantages.clone(), (batch.size,), &self.device)
            .map_err(|e| Error::Training(e.to_string()))?;
        let ret_tensor = Tensor::from_vec(returns, (batch.size,), &self.device)
            .map_err(|e| Error::Training(e.to_string()))?;

        // Normalize Advantages - must do element-wise to avoid broadcasting issues
        let adv_mean_val: f32 = adv_tensor
            .mean_all()
            .map_err(|e| Error::Training(e.to_string()))?
            .to_scalar()
            .map_err(|e| Error::Training(e.to_string()))?;
        let adv_var = adv_tensor
            .var(0)
            .map_err(|e| Error::Training(e.to_string()))?;
        let adv_std_val: f32 = adv_var
            .sqrt()
            .map_err(|e| Error::Training(e.to_string()))?
            .to_scalar()
            .map_err(|e| Error::Training(e.to_string()))?;
        ...
        let adv_norm_vec: Vec<f32> = advantages
            .iter()
            .map(|a| (a - adv_mean_val) / std_with_eps)
            .collect();
        let adv_normalized = Tensor::from_vec(adv_norm_vec, (batch.size,), &self.device)
            .map_err(|e| Error::Training(e.to_string()))?;

        // Constants Tensors
        let value_coef_tensor =
            Tensor::new(VALUE_COEF, &self.device).map_err(|e| Error::Training(e.to_string()))?;
        let entropy_coef_tensor =
            Tensor::new(ENTROPY_COEF, &self.device).map_err(|e| Error::Training(e.to_string()))?;
        let _one_tensor =
            Tensor::new(1.0f32, &self.device).map_err(|e| Error::Training(e.to_string()))?;
        ...
        for _ in 0..EPOCHS {
            ...
            let loss = policy_loss
                .add(
                    &v_loss
                        .mul(&value_coef_tensor)
                        .map_err(|e| Error::Training(e.to_string()))?,
                )
                .map_err(|e| Error::Training(e.to_string()))?
                .sub(
                    &entropy
                        .mul(&entropy_coef_tensor)
                        .map_err(|e| Error::Training(e.to_string()))?,
                )
                .map_err(|e| Error::Training(e.to_string()))?;
            ...
            self.optimizer
                .backward_step(&loss)
                .map_err(|e| Error::Training(format!("Backward: {}", e)))?;
        ...
        let result = TrainingResult::new()
            ...
            .with_metric(
                "mean_reward",
                rewards_vec.iter().sum::<f32>() as f64 / batch.size as f64,
            )
